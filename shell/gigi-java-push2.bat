...
SET myServerAddress="192.168.50.62"
SET defaultServerAddress="183.6.57.47"
...
IF "%2"=="default" (
  ...
  powershell -Command "(gc %filename% -encoding utf8) -replace '%myServerAddress%', '%defaultServerAddress%' | Out-File -encoding utf8 %filename%"
) ELSE IF "%2"=="my" (
  ...
  powershell -Command "(gc %filename% -encoding utf8) -replace '%defaultServerAddress%', '%myServerAddress%' | Out-File -encoding utf8 %filename%"
)
EXIT /B 0
